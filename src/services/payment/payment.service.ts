...
export class PaymentService implements IPaymentService {
  ...
  constructor(private readonly prisma: PrismaService) {
    ...
    this.returnUrl =
      process.env.PAYOS_RETURN_URL || 'http://localhost:3000/payments/success';
    this.cancelUrl =
      process.env.PAYOS_CANCEL_URL || 'http://localhost:3000/payments/cancel';
  }
  ...
  async createPaymentLink(dto: CreatePaymentLinkDto) {
    ...
    // Use Decimal.js to avoid floating point precision loss
    ...
    try {
      ...
      if (!checkoutUrl) {
        ...
        return {
          ...existing,
          checkoutUrl:
            existing.checkoutUrl ?? existing.data?.checkoutUrl ?? null,
          orderId: order.id,
        };
      }
      ...
      if (
        error?.response?.data?.code === '231' ||
        error?.message?.includes('231')
      ) {
        const existing = (await this.payos.paymentRequests.get(
          order.id,
        )) as any;
  ...
  async handlePayOSWebhook(webhookBody: PayOSWebhookDto) {
    ...
    try {
      ...
      throw new BadRequestException(
        `Invalid webhook signature: ${error.message || error}`,
      );
    }
    ...
    // Compare amounts as Decimal to avoid floating point precision issues
    const paymentAmount = new Decimal(
      payment.amount.toString(),
    ).toDecimalPlaces(2, Decimal.ROUND_HALF_UP);
    const webhookAmount = new Decimal(
    ...
    try {
      ...
      if (
        ...
        const latestTransaction =
          payosInfo.transactions?.[payosInfo.transactions.length - 1];
      ...
      throw new BadRequestException(
        `Failed to fetch PayOS status: ${error.message}`,
      );
    }
  }
}
