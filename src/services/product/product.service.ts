...
export class ProductService implements IProductService {
  ...
  private async registerGetAllCacheKey(cacheKey: string): Promise<void> {
    const keys =
      (await this.cacheManager.get<string[]>(this.getAllCacheKeysIndex)) ?? [];

    if (!keys.includes(cacheKey)) {
      await this.cacheManager.set(this.getAllCacheKeysIndex, [
        ...keys,
        cacheKey,
      ]);
    }
  ...
  async GetAll(
    data: PageFilterDto,
  ): Promise<PageResponseModel<ProductResponse>> {
    const startedAt = Date.now();
    data.normalize();

    const cacheKey = `products:getall:${data.page}:${data.limit}`;
    const cached =
      await this.cacheManager.get<PageResponseModel<ProductResponse>>(cacheKey);
